    ...
    const fetchData = async () => {
      if (state.data === 0 && !state.city) {
        ...
        try {
          ...
          dispatch({ type: "bd", payload: res.data });
          setSuggestMessage(
            `Estas son nuestras recomendaciones de ${state.categorySelected}`
          );
        } catch (error) {
          console.log(
            "Error al obtener datos de la API. Usando datos estáticos..."
        ...
        try {
          ...
          dispatch({ type: "bd", payload: res.data });
          setSuggestMessage(
            `Estas son nuestras recomendaciones para ${state.city} con disponiblidad entre ${newReservation[0]} y ${newReservation[1]}`
          );
        } catch (error) {
          if (error.response.status !== 200) {
            setSuggestMessage(
              `No encontramos resultados para ${state.city} en las fechas seleccionadas, pero te dejamos estas recomendaciones que pueden ser de tu interes: `
            );
            try {
              const res = await axios.get(apiProductRandom);
              dispatch({ type: "bd", payload: res.data });
